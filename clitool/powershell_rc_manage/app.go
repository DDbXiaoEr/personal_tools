...
func newModel(dir string, store *psrc.Store) model {
	return model{
		dir:     dir,
		store:   store,
		tab:     psrc.KindAlias,
...
func (m *model) file() *psrc.File {
	return m.store.File(m.tab)
}

func (m *model) items() []*psrc.Item {
	return m.file().Items
}
...
func (m *model) rows() []listRow {
	items := m.items()
	var out []listRow
	for _, cat := range psrc.CategoryOrder(items) {
...
func (m *model) currentItem() *psrc.Item {
	items := m.items()
...
func (m model) updateList(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "q", "ctrl+c":
		...
		return m, tea.Quit
	case "1":
		m.tab = psrc.KindAlias
		m.clampCursor()
	case "2":
		m.tab = psrc.KindEnv
		m.clampCursor()
	case "3":
		m.tab = psrc.KindFunctions
		m.clampCursor()
	case "tab":
		m.nextTab()
	case "shift+tab":
...
func (m *model) reload() {
	if m.anyDirty() {
		m.setErr("有未保存的修改，请先保存或放弃后再重载")
		return
	}
	store, err := psrc.Load(m.dir)
	if err != nil {
		m.setErr("重载失败: " + err.Error())
		return
	}
	m.store = store
	m.dirty = map[psrc.Kind]bool{}
	m.clampCursor()
	m.setOK("已从文件重载")
}
...
func (m *model) buildForm(it *psrc.Item) {
	cat := ""
	if it != nil {
		cat = it.Category
		if cat == psrc.Uncategorized {
			cat = ""
		}
	} else if cur := m.currentItem(); cur != nil && cur.Cat() != psrc.Uncategorized {
		cat = cur.Cat()
	}
	...
	case psrc.KindEnv:
		...
		styleIdx := 0
		if it != nil && it.Style == psrc.StyleSnippet {
			styleIdx = 1
		}
		category := newText("分类", "Go")
		style := newChoice("类型", []string{"env", "snippet"}, styleIdx)
		name := newText("名称", "PATH")
		value := newText("值", `$env:PATH;C:\bin`)
		body := newArea("内容", `if (Test-Path $nvm) { . $nvm }`, 8)
		if it != nil {
			category.setText(cat)
			name.setText(it.Name)
			if it.Name != "" {
	...
	case psrc.KindFunctions:
		...
		name := newText("名称", "ip")
		body := newArea("函数体", "Write-Host hi", 10)
		if it != nil {
			category.setText(cat)
			name.setText(it.Name)
			name.readOnly = true
...
func (m *model) itemFromForm() (*psrc.Item, error) {
	switch m.tab {
	case psrc.KindAlias:
		...
		return &psrc.Item{
			...
			Name:     name,
			Value:    cmd,
			Style:    psrc.StyleAlias,
			Quote:    inferQuote(cmd),
		}, nil
	case psrc.KindEnv:
	...
	default:
		...
		return &psrc.Item{
			...
			Name:     name,
			Value:    body,
			Style:    psrc.StyleFunction,
		}, nil
	}
}

func inferQuote(v string) psrc.Quote {
	if strings.ContainsAny(v, "$") {
		return psrc.QuoteDouble
	}
	if v == "" || strings.ContainsAny(v, " \t") {
		return psrc.QuoteSingle
	}
	return psrc.QuoteNone
}
...
func (m model) headerView() string {
	...
	return titleStyle.Render("psman") + "  " + strings.Join(parts, "") + dimStyle.Render("  "+path+mark)
}
...
func styleTag(it *psrc.Item) string {
	if it.Style == psrc.StyleSnippet {
...
func (m model) helpLine() string {
	return "1/2/3 切换 · a 新增 · enter/e 编辑 · d 删除 · s 保存 · r 重载 · ? 帮助 · q 退出"
}

func (m model) helpView() string {
	return dimStyle.Render(
		"提示：\n" +
			"  · 文件：~/.ps_{alias,env,functions}.ps1，按分类注释分组写出。\n" +
			"  · 修改先留在内存，按 s 只写回当前标签对应文件，并生成 .bak。\n" +
			"  · Alias 对应 Set-Alias，只能指向命令名；带参数请用 Functions。\n" +
			"  · Env 类型：env（$env:KEY = value）/ snippet（多行脚本）。\n" +
			"  · 函数体和 snippet 在表单 textarea 中编辑，ctrl+s 应用。\n" +
			"  · 本工具不修改 $PROFILE，请自行 . ~/.ps_alias.ps1 等。")
}
...
func nameColWidth(items []*psrc.Item, total int) int {
	w := 8
